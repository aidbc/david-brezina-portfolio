# Texty webu

Pracovní verze 1. Texty v hranatých závorkách jsou poznámky nebo informace k doplnění. Na web se nemají propsat.

## Tón textů

- psát v první osobě a jako při běžném pracovním rozhovoru,
- používat krátké věty a konkrétní slovesa,
- nepřidávat superlativy ani tvrzení, která nelze doložit,
- nezahlcovat text anglickými product-management výrazy, pokud existuje přirozená česká formulace,
- u projektů jasně odlišit moji práci od práce celého týmu,
- pokud výsledek neznáme nebo nemůžeme zveřejnit, přiznat to nebo danou větu vynechat.

## Zjednodušená struktura

1. Úvod
2. Vybrané projekty
3. Zkušenosti
4. Jak pracuji
5. O mně
6. Kontakt

Samostatná sekce „Co přináším“ není potřeba. Stejné věci mají být vidět z projektů. AI také nepotřebuje vlastní sekci; objeví se tam, kde je součástí skutečné práce.

---

# Čeština

## Navigace

- Projekty
- Zkušenosti
- Jak pracuji
- O mně
- Kontakt
- EN
- Stáhnout profil (.md)

## Úvod

### Jméno

David Březina

### Role

Product Lead

### Headline

Pomáhám dostat nové digitální produkty od nápadu ke spuštění.

### Úvodní text

Vedu discovery, pomáhám najít smysluplné MVP a propojuji byznys, design a vývoj tak, aby se produkt skutečně dostal na trh.

Mám přes 10 let zkušeností, především z fintechu, bankovnictví a wealth managementu.

### Odkazy

- Prohlédnout projekty
- Pracovní zkušenosti
- Stáhnout profil (.md)
- LinkedIn

### Rychlá fakta

- 10+ let v digitálních produktech
- Od discovery k launchi
- Fintech a wealth management
- Praha, práce v CZ/SK/EU

## Vybrané projekty

### Nadpis

Vybrané projekty

### Úvod

Na projektech mě zajímá hlavně cesta od nejasného zadání ke konkrétnímu produktu. Tady jsou tři příklady.

### Projekt 1: Mobilní investiční aplikace Across Wealth

**Období**

únor 2026 – červenec 2026

**Krátký popis**

Nová regulovaná investiční aplikace pro začínající a mass-affluent investory. Projekt rozšířil původně webovou platformu Across o nativní aplikaci pro iOS a Android.

**Moje role**

Product Owner

**Co jsem dělal**

Vedl jsem product discovery a delivery mobilní aplikace. Definoval jsem MVP, klíčové user flows a user stories, vlastnil backlog a průběžně slaďoval byznys, design a vývoj.

**Výsledek**

Aplikaci jsme v plánovaném termínu dostali do App Storu a Google Play. Složitou oblast investic, custody a compliance jsme převedli do mobilního prostředí navazujícího na existující web. Vznikl tím také základ pro nový investiční pilíř Across Crowd.

**Štítky**

Wealth management · Mobile · Delivery

**Odkaz**

Detail projektu

**Veřejný odkaz**

https://across.sk

### Projekt 2: Wealth Management Product Discovery

**Období**

září 2025 – leden 2026

**Krátký popis**

Návrh nové wealth management části existující fintech aplikace. Cílem bylo rozšířit původní investiční nabídku o další typy produktů a zachovat přitom jednoduché uživatelské prostředí.

**Moje role**

Product Lead / Product Manager

**Co jsem dělal**

Vedl jsem produktovou část discovery. Definoval jsem MVP, user flows, produktové požadavky a backlog a slaďoval práci byznysu, designu a vývoje.

**Výsledek**

Výstupem byla user story mapa, roadmapa a klikací high-fidelity prototyp. Navrhli jsme strukturu, která dokáže spojit různé typy investičních produktů bez toho, aby aplikace pro uživatele zbytečně zesložitěla.

**Štítky**

Wealth management · Discovery · MVP

**Odkaz**

Detail projektu

### Projekt 3: Stealth Trading App

**Období**

červenec 2024 – leden 2025

**Krátký popis**

Discovery a vývoj AI-powered mobilní investiční aplikace pro akcie a ETF s prvky social tradingu.

**Moje role**

Product Lead, později Product Owner

**Co jsem dělal**

Projekt začal čtyřtýdenní discovery, během které jsem vedl průzkum trhu, rozhovory s uživateli a analýzu konkurence. Společně s týmem jsme definovali value proposition AI-powered investiční aplikace pro akcie a ETF, její cílové uživatele a rozsah MVP. Následovalo přibližně půl roku vývoje první verze aplikace, během něhož jsem působil jako Product Lead a Product Owner.

**Výsledek**

Za čtyři týdny jsme ověřili hlavní produktové koncepty a připravili konkrétní plán. Během navazujícího přibližně půlročního vývoje jsme vytvořili první verzi aplikace.

**Štítky**

Investments · Discovery · AI

**Odkaz**

Detail projektu

## Zkušenosti

### Nadpis

Zkušenosti

### Vacuumlabs

**Senior Product Manager / Product Lead**  
listopad 2023 – 31. srpna 2026 · freelance · Praha

Pracoval jsem na klientských projektech ve fintechu a wealth managementu. Vedl jsem discovery i delivery a pomáhal s definicí nových produktů, MVP a plánem realizace. Jako Product Lead pro Wealth Management jsem se podílel také na strategii a práci s klienty.

Uvnitř Vacuumlabs jsem se věnoval využití AI v produktových a delivery týmech.

Související projekty:

- Mobilní investiční aplikace Across Wealth
- Wealth Management Product Discovery
- Kreditní karta pro britskou credit union
- B2B portál pro poskytovatele úvěrů
- Stealth Trading App
- Product management coaching ve Slovenské spořitelně

### Finbricks

**Product Owner**  
leden 2023 – listopad 2023 · freelance · Praha

Vedl jsem produktový vývoj open banking platformy pro český a slovenský trh. Připravoval jsem nové funkce, pracoval s backlogem a pomáhal týmu zlepšit způsob delivery.

Dodali jsme novou administraci pro klienty včetně migrace a podpořili vstup na maďarský a rumunský trh.

### Shoptet

**Product Manager, Shoptet Pay**  
listopad 2021 – prosinec 2022 · freelance · Praha

Podílel jsem se na budování Shoptet Pay pro největší e-commerce platformu v regionu CEE. Vedl jsem integraci open bankingu pro automatické párování plateb s objednávkami a pomohl nastavit způsob práce produktového týmu.

### Creative Dock

**Product Owner / Project Lead**  
září 2019 – listopad 2021 · freelance · Praha

Ve Fairo jsem jako Product Owner vedl produktový tým zaměřený na fakturaci, účetnictví a daně pro živnostníky. Později jsem vedl celý projekt digitální agentury pro malé firmy, který vznikal ve spolupráci s KB a Visa.

U mobilní banky jsme dodali například biometrické podepisování, chat, notifikační centrum a úpravy daňového reportingu. Digitální agenturu jsme postavili od nuly a MVP spustili podle plánu.

### Česká spořitelna

**Product & CX Specialist**  
leden 2019 – září 2019 · Praha

Vedl jsem delivery plně online cestovního pojištění v George Marketplace.

**Product Owner / Senior Business Analyst**  
říjen 2016 – prosinec 2018 · Praha

Pracoval jsem na digitální transformaci banky a přechodu 1,8 milionu klientů na nové internetové a mobilní bankovnictví. Jako Product Owner aplikace George klíč jsem koordinoval architekturu, UX, bezpečnost, risk a compliance, legal, IT provoz i byznysové vlastníky.

### Dřívější zkušenosti

- T-Mobile Czech Republic, Process Manager, 2015–2016
- KPMG Česká republika, Business Analyst → Consultant, 2012–2015
- Deloitte Česká republika, Audit Assistant, 2010–2011

## Jak pracuji

### Nadpis

Od prvního nápadu k fungujícímu produktu

### Úvod

Každý projekt je jiný, ale většinou procházím těmito kroky:

### 1. Pochopit problém

Nejdřív potřebuji vědět, pro koho produkt vzniká, co má změnit a které předpoklady zatím nejsou ověřené.

### 2. Najít správné MVP

Společně s týmem oddělíme to, co je pro první verzi podstatné, od věcí, které mohou počkat.

### 3. Sladit lidi kolem produktu

Hlídám, aby byznys, design, vývoj a další stakeholdeři rozuměli cíli stejně a rozhodovali se nad stejnými informacemi.

### 4. Dovést produkt k realizaci

Převádím směr produktu do konkrétních flows, priorit a backlogu. Během delivery řeším změny scope a pomáhám odstraňovat blokace.

### 5. Spustit a poučit se

Launch není konec projektu. Zpětná vazba a data ukážou, co má smysl rozvíjet dál.

## O mně

### Nadpis

O mně

### Text

Jsem Product Lead s více než 10 lety zkušeností s digitálními produkty. Pracoval jsem ve startupech, technologických firmách i velké bance. Nejvíc mi vyhovují situace, kdy ještě není všechno rozhodnuté a produkt potřebuje dostat jasný tvar.

V posledních letech se zaměřuji hlavně na fintech a wealth management. Vedle klientských projektů zkoumám, kde může AI produktovým týmům skutečně ušetřit práci a kde naopak přidává jen další vrstvu složitosti.

### Oblasti

- Product discovery
- New product development
- MVP definition
- Product delivery
- Wealth management
- Open banking a platby
- Digitální identita
- AI workflow pro produktové týmy

### Jazyky

- Čeština, rodilý mluvčí
- Angličtina, pokročilá

### Nástroje

Jira, Confluence, Figma, Miro, GitHub, n8n, Metabase, Mixpanel, Hotjar a LLM nástroje.

### Vzdělání a certifikace

- Univerzita Hradec Králové, Informační management
- Universidad Politécnica de Madrid, Erasmus
- Professional Scrum Product Owner I, Scrum.org

## Kontakt

### Nadpis

Pojďme se spojit

### Text

Jsem otevřený spolupráci na IČO i pracovnímu poměru. Záleží na projektu a vzájemné dohodě.

### Odkazy

- brezina.david@gmail.com
- +420 603 424 391
- https://www.linkedin.com/in/davidbrezina/
- Stáhnout profil (.md)

### Poznámka k Markdownu

Textovou verzi profilu si můžete stáhnout a vložit například do ChatGPT nebo Claude, pokud se chcete na moje zkušenosti zeptat podrobněji.

## Footer

David Březina · Praha · 2026

Obsah naposledy aktualizován 19. srpna 2026.

---

# English

## Navigation

- Projects
- Experience
- How I work
- About
- Contact
- CZ
- Download profile (.md)

## Introduction

### Name

David Březina

### Role

Product Lead

### Headline

I help turn new product ideas into launched digital products.

### Introduction text

I lead discovery, help teams find the right MVP, and bring business, design and engineering together to get products to market.

I have over 10 years of experience, mainly across fintech, banking and wealth management.

### Links

- View projects
- Work experience
- Download profile (.md)
- LinkedIn

### Quick facts

- 10+ years in digital products
- From discovery to launch
- Fintech and wealth management
- Prague, experience across CZ/SK/EU

## Selected projects

### Introduction

The part I enjoy most is turning an unclear brief into a product a team can build. Here are three examples.

### Project 1: Across Wealth Mobile Investment App

**Period**

February 2026 – July 2026

**Short description**

A new regulated investment application for first-time and mass-affluent investors. The project extended Across from a web-only platform into native iOS and Android applications.

**My role**

Product Owner

**What I did**

I led product discovery and delivery for the mobile application. I defined the MVP, key user flows and user stories, owned the backlog, and aligned business, design and engineering throughout delivery.

**Outcome**

We shipped the application on schedule and received approval from both Apple and Google. We turned a complex investment, custody and compliance setup into a mobile experience consistent with the existing web product. The result also became a foundation for the new Across Crowd investment pillar.

**Tags**

Wealth management · Mobile · Delivery

**Public link**

https://across.sk

### Project 2: Wealth Management Product Discovery

**Period**

September 2025 – January 2026

**Short description**

Defining a new wealth management section within an existing fintech application. The goal was to broaden the original investment proposition while keeping the user experience simple.

**My role**

Product Lead / Product Manager

**What I did**

I led the product side of the discovery. I defined the MVP, user flows, product requirements and backlog, and aligned business, design and engineering.

**Outcome**

The work produced a user story map, roadmap and clickable high-fidelity prototype. We designed a structure that could bring different types of investment products together without making the application unnecessarily complex for users.

**Tags**

Wealth management · Discovery · MVP

### Project 3: Stealth Trading App

**Period**

July 2024 – January 2025

**Short description**

Discovery and development of an AI-powered mobile investment app for stocks and ETFs with social-trading features.

**My role**

Product Lead, later Product Owner

**What I did**

The project began with a four-week discovery, during which I led market research, user interviews and competitor analysis. Together with the team, we defined the value proposition, target users and MVP scope for an AI-powered investment app for stocks and ETFs. This was followed by roughly six months of development of the first version, where I worked as Product Lead and Product Owner.

**Outcome**

In four weeks, we validated the core product concepts and created an actionable plan. Over the following six months, we developed the first version of the application.

**Tags**

Investments · Discovery · AI

## Experience

### Vacuumlabs

**Senior Product Manager / Product Lead**  
November 2023 – 31 August 2026 · freelance · Prague

I worked on client projects in fintech and wealth management, across both discovery and delivery. As Product Lead for Wealth Management, I also supported strategy and client work.

Internally, I worked on practical uses of AI for product and delivery teams.

Related projects:

- Across Wealth Mobile Investment App
- Wealth Management Product Discovery
- Credit card for a UK credit union
- B2B lending portal
- Stealth Trading App
- Product management coaching at Slovenská sporiteľňa

### Finbricks

**Product Owner**  
January 2023 – November 2023 · freelance · Prague

I led product development for an open banking platform in the Czech and Slovak markets. I worked on new features, owned the backlog and helped the team improve delivery.

We delivered a new client administration portal, including migration, and supported expansion into Hungary and Romania.

### Shoptet

**Product Manager, Shoptet Pay**  
November 2021 – December 2022 · freelance · Prague

I helped build Shoptet Pay for the largest e-commerce platform in Central and Eastern Europe. I led the open banking integration used to match payments with orders and helped establish the product team's way of working.

### Creative Dock

**Product Owner / Project Lead**  
September 2019 – November 2021 · freelance · Prague

At Fairo, I worked as Product Owner for a team focused on invoicing, bookkeeping and taxation for freelancers. I later led a digital agency project for small businesses, created with KB and Visa.

For the mobile bank, we delivered biometric signing, in-app chat, a notification centre and improvements to tax reporting. We built the digital agency from scratch and launched the MVP on schedule.

### Česká spořitelna

**Product & CX Specialist**  
January 2019 – September 2019 · Prague

I led the delivery of fully online travel insurance in George Marketplace.

**Product Owner / Senior Business Analyst**  
October 2016 – December 2018 · Prague

I worked on the bank's digital transformation and the migration of 1.8 million clients to new online and mobile banking. As Product Owner for George Key, I coordinated architecture, UX, security, risk and compliance, legal, IT operations and business owners.

### Earlier experience

- T-Mobile Czech Republic, Process Manager, 2015–2016
- KPMG Czech Republic, Business Analyst → Consultant, 2012–2015
- Deloitte Czech Republic, Audit Assistant, 2010–2011

## How I work

### Heading

From the first idea to a working product

### 1. Understand the problem

I start by finding out who the product is for, what should change, and which assumptions are still untested.

### 2. Find the right MVP

Together with the team, I separate what matters for the first version from what can wait.

### 3. Align the people around the product

I make sure business, design, engineering and other stakeholders understand the goal in the same way and make decisions using the same information.

### 4. Turn the direction into delivery

I translate the product direction into flows, priorities and a backlog. During delivery, I manage scope changes and help remove blockers.

### 5. Launch and learn

Launch is not the end of the project. Feedback and data show what is worth developing next.

## About

I am a Product Lead with more than 10 years of experience in digital products. I have worked in startups, technology companies and a large bank. I am at my best when the answer is not yet obvious and a product needs a clear shape.

In recent years, I have focused mainly on fintech and wealth management. Alongside client work, I explore where AI genuinely saves product teams time and where it simply adds another layer of complexity.

### Areas

- Product discovery
- New product development
- MVP definition
- Product delivery
- Wealth management
- Open banking and payments
- Digital identity
- AI workflows for product teams

### Languages

- Czech, native
- English, advanced

### Tools

Jira, Confluence, Figma, Miro, GitHub, Metabase, Mixpanel, Hotjar

### AI Stack
Claude Code, Codex, Github, Lovable, Linear

### Education and certification

- University of Hradec Králové, Information Management
- Universidad Politécnica de Madrid, Erasmus
- Professional Scrum Product Owner I, Scrum.org

## Contact

### Heading

Let's talk

### Text

I am open to both freelance contracts and permanent employment, depending on the role and mutual agreement.

### Links

- brezina.david@gmail.com
- +420 603 424 391
- https://www.linkedin.com/in/davidbrezina/
- Download profile (.md)

### Markdown note

You can download a text version of this profile and add it to ChatGPT or Claude if you want to explore my experience in more detail.

## Footer

David Březina · Prague · 2026

Content last updated 19 August 2026.
